...
if check_password():
        ...
        with t2:
            if "model" not in st.session_state:
                ...
                if batch_file:
                    ...
                    batch_df['Predicted_Risk'] = st.session_state["model"].predict(X_batch)
    ...
    # --- Page 4: Patient Diagnosis ---
        ...
        if "model" in st.session_state:
            ...
            with display_col:
                # FIX: Ensure we extract probability as a standard Python float to prevent Plotly errors
                ...
                try:
                    ...
                    fig.update_layout(height=450, margin=dict(l=30, r=30, t=80, b=30))
                    st.plotly_chart(fig, use_container_width=True)
                ...
                st.markdown(f"""
                    <div style='background-color:{status_color}; padding:20px; border-radius:10px; text-align:center;'>
                        ...
                        <p style='color:white; margin:0; opacity:0.9;'>System Confidence: {risk_prob:.1f}%</p>
                    </div>
                ...
                if st.button("💾 Log Verified Diagnosis"):
                    log_action("Individual Diagnosis", f"Risk: {risk_prob:.1f}%")
                    st.toast("Clinical Log Updated", icon="🏥")
        else:
            st.warning("Train model in Step 2 first.")
